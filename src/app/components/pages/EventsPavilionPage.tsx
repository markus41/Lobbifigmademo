...
const CalendarIcon = ({ className, style }: { className?: string; style?: CSSProperties }) => (
...
const ClockIcon = ({ className, style }: { className?: string; style?: CSSProperties }) => (
  <svg className={className} style={style} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
function EventCard({ event, organization: _organization, onClick, onRegister }: EventCardProps) {
  const typeColors = {
    conference: '#9333EA',
    workshop: '#0EA5E9',
    webinar: '#22C55E',
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        '--tw-ring-color': event.featured ? 'var(--theme-primary)' : undefined,
        borderColor: 'var(--theme-border-light, #EDE8DD)',
      } as CSSProperties}
      whileHover={{ y: -4, boxShadow: 'var(--theme-shadow-lg, 0 12px 32px rgba(0,0,0,0.1))' }}
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          {/* Click Count Badge */}
          <div className="flex items-center gap-1 text-xs" style={{ color: 'var(--theme-text-muted, #6B7280)' }}>
        ...
        {/* Title & Description */}
        <h3 className="font-semibold mb-2 line-clamp-2" style={{ color: 'var(--theme-text-primary, #1A1A2E)' }}>{event.title}</h3>
        <p className="text-sm line-clamp-2 mb-4" style={{ color: 'var(--theme-text-muted, #6B7280)' }}>{event.description}</p>

        {/* Details */}
        <div className="space-y-2 mb-4">
          <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--theme-text-secondary, #6B7280)' }}>
            <CalendarIcon className="w-4 h-4" style={{ color: 'var(--theme-text-muted, #9CA3AF)' }} />
            <span>{new Date(event.date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</span>
          </div>
          <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--theme-text-secondary, #6B7280)' }}>
            <ClockIcon className="w-4 h-4" style={{ color: 'var(--theme-text-muted, #9CA3AF)' }} />
            <span>{event.time}</span>
          </div>
          <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--theme-text-secondary, #6B7280)' }}>
            <MapPinIcon className="w-4 h-4" style={{ color: 'var(--theme-text-muted, #9CA3AF)' }} />
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between text-sm mb-1.5">
            ...
            <span
              className={`font-medium ${isAlmostFull ? 'text-amber-600' : ''}`}
              style={!isAlmostFull ? { color: 'var(--theme-text-secondary, #6B7280)' } : undefined}
            >
              {capacityPercent}% Full
            </span>
          </div>
          <div className="h-2 rounded-full overflow-hidden" style={{ background: 'var(--theme-bg-tertiary, #F3F4F6)' }}>
            <motion.div
              className="h-full rounded-full"
              style={{
                background: isAlmostFull
                  ? 'linear-gradient(90deg, #F59E0B, #DC2626)'
                  : 'var(--theme-gradient-btn)',
              }}
              initial={{ width: 0 }}
        ...
        <button
          ...
          style={{
            background: capacityPercent >= 100 ? 'var(--theme-bg-disabled, #E5E7EB)' : 'var(--theme-gradient-btn)',
            color: capacityPercent >= 100 ? 'var(--theme-text-disabled, #9CA3AF)' : 'var(--theme-text-inverse, white)',
          }}
...
const CloseIcon = ({ className, style }: { className?: string; style?: CSSProperties }) => (
  <svg className={className} style={style} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex gap-2">
          {['all', 'conference', 'workshop', 'webinar', 'social', 'meeting'].map((type) => (
            <button
              ...
              style={{
                background: filterType === type ? 'var(--theme-gradient-btn)' : 'transparent',
                color: filterType === type ? 'var(--theme-text-inverse, white)' : 'var(--theme-text-secondary, #6B7280)',
              }}
            >
        ...
        <div className="flex rounded-lg p-1" style={{ background: 'var(--theme-bg-tertiary, #F3F4F6)' }}>
          <button
            className="p-2 rounded-md transition-colors"
            style={{
              background: viewMode === 'grid' ? 'var(--theme-bg-card, #FFFFFF)' : 'transparent',
              boxShadow: viewMode === 'grid' ? 'var(--theme-shadow-sm, 0 1px 2px rgba(0,0,0,0.05))' : 'none',
            }}
            onClick={() => setViewMode('grid')}
          >
            <GridIcon className="w-4 h-4" style={{ color: viewMode === 'grid' ? 'var(--theme-primary, #D4AF37)' : 'var(--theme-text-muted, #8A8578)' }} />
          </button>
          <button
            className="p-2 rounded-md transition-colors"
            style={{
              background: viewMode === 'list' ? 'var(--theme-bg-card, #FFFFFF)' : 'transparent',
              boxShadow: viewMode === 'list' ? 'var(--theme-shadow-sm, 0 1px 2px rgba(0,0,0,0.05))' : 'none',
            }}
            onClick={() => setViewMode('list')}
          >
            <ListIcon className="w-4 h-4" style={{ color: viewMode === 'list' ? 'var(--theme-primary, #D4AF37)' : 'var(--theme-text-muted, #8A8578)' }} />
          </button>
        </div>
      </div>
      ...
      {filteredEvents.length === 0 && (
        <div className="text-center py-12">
          <CalendarIcon className="w-12 h-12 mx-auto mb-4" style={{ color: 'var(--theme-text-disabled, #D1D5DB)' }} />
          <p style={{ color: 'var(--theme-text-muted, #6B7280)' }}>No events found for this category.</p>
        </div>
      )}
        ...
        {showEventBuilder && (
          ...
          >
            ...
            <motion.div
              ...
              style={{
                background: 'var(--theme-bg-card, #FFFFFF)',
                boxShadow: 'var(--theme-shadow-xl, 0 25px 50px -12px rgba(0,0,0,0.25))',
              }}
            ...
            >
              ...
              <button
                ...
                style={{ background: 'var(--theme-bg-tertiary, #F3F4F6)' }}
              >
                <CloseIcon className="w-5 h-5" style={{ color: 'var(--theme-text-secondary, #6B7280)' }} />
              </button>
